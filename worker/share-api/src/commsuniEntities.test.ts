...
import { entityReference, lookupTvdbId, rememberTvdbId, resolveReference } from "./commsuniEntities";
...
describe("entityReference", () => {
  it("builds the four shapes the archive uses", () => {
    expect(entityReference("show", 121361)).toBe("tvdb-121361");
    expect(entityReference("show", 121361, 2)).toBe("tvdb-121361-s2");
    expect(entityReference("show", 121361, 2, 5)).toBe("tvdb-121361-s2e5");
    expect(entityReference("movie", 12345)).toBe("movie/tvdb-12345");
  });

  it("⚠️ namespaces a MOVIE, because series and movie ids overlap numerically", () => {
    // Without the prefix this addresses series 12345's conversation instead — a
    // well-formed reference to entirely the wrong title, with no error anywhere.
    expect(entityReference("movie", 12345)).not.toBe("tvdb-12345");
  });
  ...
  it("treats the -1 sentinels as 'level does not apply', never as a real season", () => {
    ...
    expect(entityReference("show", 99, -1, -1)).toBe("tvdb-99");
    expect(entityReference("show", 99, 3, -1)).toBe("tvdb-99-s3");
    // A movie can never carry a level, even if one is somehow passed.
    expect(entityReference("movie", 99, 3, 4)).toBe("movie/tvdb-99");
  });
...
describe("resolveReference", () => {
  it("prefers the client's id AND teaches the map, so the server can address it later", async () => {
    ...
    expect(ref).toBe("tvdb-121361-s2e5");
  ...
  it("falls back to the cache when the client sends nothing", async () => {
    ...
    expect(await resolveReference(e, "movie", 550, -1, -1, null)).toBe("movie/tvdb-777");
  });
